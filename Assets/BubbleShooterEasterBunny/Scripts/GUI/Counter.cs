...
public class Counter : MonoBehaviour {
  //  UILabel label;
	...
	void Update () {
        ...
        if (name == "TotalScore" && label.text!="")
        {
            if (ColyseusClient.Instance.room.State.player1.sessionId == ColyseusClient.Instance.room.SessionId)
            {

                label.text = (ColyseusClient.Instance.room.State.player1.TotalScore+ mainscript.Score).ToString();
            ...
            else
            {
                ...
                label.text = (ColyseusClient.Instance.room.State.player2.TotalScore + mainscript.Score).ToString();
