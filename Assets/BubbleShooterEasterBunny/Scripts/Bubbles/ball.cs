   ...
   /* private void OnTriggerEnter2D(Collider other)
    ...
    public void checkNearestColor()
    {
        ...
        foreach (GameObject obj in fixedBalls)
        {
           ...
           else */if ((obj.layer==9 && /*(BallColor)obj.GetComponent<bouncer>().DoubleColor==(BallColor)System.Enum.Parse(typeof(BallColor),tag)*/ obj.GetComponent<bouncer>().ballcolors.Contains((BallColor)System.Enum.Parse(typeof(BallColor), tag)) && obj.GetComponent<bouncer>().bGumNo == 0) || obj.gameObject.tag == tag)
            {
//                Debug.Log(tag + "==doyble=" + obj.gameObject.GetComponent<bouncer>().DoubleColor + "------------obj value" + obj.gameObject.tag);
                float distTemp = Vector3.Distance(transform.position, obj.transform.position);
  ...
  /*  public bool checkNearestBall( ArrayList b )
            ...
            if( !other.gameObject.GetComponent<ball>().enabled )
            {
                ...
                if (other.gameObject.tag== "rowRemover") // code for removing row of row remover bubble
                {
                    ...
                    foreach(GameObject obj in fixedBalls)
                    {
                        ...
                        else if (obj.layer == 9 && obj.GetComponent<Transform>().position.y < rowToRemove)
                ...
                if (other.GetComponent<bouncer>().bGumNo != 0)
                {
                ...
                if (other.gameObject.tag == "frozen") {// Code for changing random color and destroying shooter bubble
                ...
                if (other.gameObject.tag == "eater") //logic for eating bubble...
    ...
    void MoveBubbles1RowDown(float topRow )
    {
       ...
       /* List<int[]> patternoMove = new List<int[]>();
        ...
        //2 RowAdding Ball working perfectly
