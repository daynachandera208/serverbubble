...
public class mainscript : MonoBehaviour {
    ...
    public int chickenRoteteInMoves;
    public int multiColorAfterMoves;
    public int totalmulticolorspans;
    public int multiColorProbability;
    public int moveCountForMultiColorBall;
    public int bombAfterMoves;
    public int totalbombspans;
    public int bombProbability;
    public int moveCountForbomb;
    public int fireballAfterMoves;
    public int totalfireballspans;
    public int fireballProbability;
    public int moveCountForfireball;
    ...
    {
        Score += value;
        if (!ColyseusClient.Instance.isoffline)
        ColyseusClient.Instance.SendMessage("NewScore:-"+Score);
        // Debug.Log("Score Incresed Here...! from mainScript only...!"+Score);
    ...
    public void PowerUPCall()
    {
        ...
        //  Invoke("GetBubbleGumSpred",2f);
        if (GamePlay.Instance.doesBubbleSpread)
        {
            GameObject[] fixedBalls = GameObject.FindObjectsOfType(typeof(GameObject)) as GameObject[];

            GamePlay.Instance.candidatesForSpreadingBGum = new GameObject[100];
            //  Debug.Log("adding bubbles ma haji locho 6 j nathi");
            // for (int i = 0; i < 1000; i++) { Debug.Log(GamePlay.Instance.candidatesForSpreadingBGum[i]); }
            foreach (GameObject obj in fixedBalls)
            {
                if (obj.layer == 9 && obj.GetComponent<bouncer>().bGumNo != 0)
                {
                    obj.GetComponent<bouncer>().CheckNearestForBubbleGum();
                }
            }
            Invoke("GetBubbleGumSpred", 1f);
            // bouncer.SpreadBubbleGum();
        }
        ...
        /*if (multiColorProbability > 0)
        {
            moveCountForMultiColorBall = moveCountForMultiColorBall + 1;
    ...
    void Update() {
        ...
        if (checkBall != null && (GamePlay.Instance.GameStatus == GameState.Playing || GamePlay.Instance.GameStatus == GameState.WaitForChicken))
        {
            ...
            LimitAmmountMinusBy1();
            // print("//////"+moveCountForMultiColorBall);
            this.PowerUPCall();
            checkBall = null;
        ...
        if (score >= LevelData.star1 && stars <= 0)
        {
            stars = 1;
            if(!ColyseusClient.Instance.isoffline)
            ColyseusClient.Instance.SendMessage("NewStar:-" + stars);
        }
	...
	IEnumerator getBallsForMesh(){
		...
		foreach(GameObject obj1 in meshes) {    											
			...
			foreach(Collider2D obj in fixedBalls) {
              //  print("adding --"+obj.gameObject); here eater ball prob is solved by if
                if (obj.gameObject.GetComponent<bouncer>().isEaterBubble==false) {
                    obj1.GetComponent<Grid>().Busy = obj.gameObject;
                    //print("adding inside logic --" + obj.gameObject);
	...
	public GameObject createFirstBall(Vector3 vector3){
        ...
        GameObject gm = GameObject.Find ("Creator");
        if (moveCountForMultiColorBall == multiColorAfterMoves ) {
            moveCountForMultiColorBall = 0;
            float randomizer = Random.value;
         //   Debug.Log(randomizer+"---"+ (1f-(multiColorProbability / 100f)));
            if (randomizer > (1f - (multiColorProbability / 100f)) && totalmulticolorspans>0 ) {
                // multiColorProbability = 0;
                //Debug.Log("last multicolor");
                ismulticolor = true;
        ...
        if (moveCountForbomb == bombAfterMoves)
        {
            moveCountForbomb = 0;
            float randomizer = Random.value;
            //   Debug.Log(randomizer+"---"+ (1f-(multiColorProbability / 100f)));
            if (randomizer > (1f - (bombProbability / 100f)) && totalbombspans > 0)
            {
                // multiColorProbability = 0;
                //Debug.Log("last multicolor");
                isbomb=true;
        ...
        if (moveCountForfireball == fireballAfterMoves)
        {
            moveCountForfireball = 0;
            float randomizer = Random.value;
            //   Debug.Log(randomizer+"---"+ (1f-(multiColorProbability / 100f)));
            if (randomizer > (1f - (fireballProbability / 100f)) && totalfireballspans > 0)
            {
                // multiColorProbability = 0;
                //Debug.Log("last multicolor");
                isfireball=true;
	...
	public void connectNearBallsGlobal(){
		...
		fixedBalls = GameObject.FindObjectsOfType(typeof(GameObject)) as GameObject[];
		foreach(GameObject obj in fixedBalls) {
            if (obj.layer == 9 /*&& !obj.GetComponent<bouncer>().isShooterBubbleDisabled */)// && obj.gameObject.tag!= "stone" && obj.GetComponent<bouncer>().bGumNo == 0)
                obj.GetComponent<ball>().connectNearBalls();
            //else if (obj.layer == 9 && obj.GetComponent<bouncer>().isShooterBubbleDisabled)
               //obj.GetComponent<bouncer>().isShooterBubbleDisabled = false;
		}
...
//            print( "check free chicken " +  balls.transform.childCount);
	...
	public void destroy( ArrayList b){
        ...
        for (int i = 0; i < 100; i++)
        {
            if (b.Contains(GamePlay.Instance.candidatesForSpreadingBGum[i])/*GamePlay.Instance.candidatesForSpreadingBGum[i] == gameObject*/) { GamePlay.Instance.candidatesForSpreadingBGum[i] = null; Debug.Log("b contains our candidate::" + i + "-----" + GamePlay.Instance.candidatesForSpreadingBGum[i]); };
        }
       // Debug.Log("Called for destruction");
        foreach (GameObject obj in b) {
//			obj.GetComponent<OTSprite>().collidable = false;
			if(obj.name.IndexOf("ball")==0) obj.layer = 0;
			if(!obj.GetComponent<ball>().Destroyed ){
                ...
                //    obj.GetComponent<ball>().growUp();
                //		soundPool++;
                if (!obj.GetComponent<ball>().Destroyed && obj.GetComponent<bouncer>().bGumNo == 0 && obj.GetComponent<bouncer>().chainNo == 0 /*&& obj.GetComponent<bouncer>().isMultiColor == false */&& !System.Array.Exists(GamePlay.Instance.avoidToChangeInMulticolor, element => element == obj.tag.ToString()))
...
//		boxFirst.GetComponent<Grid>().Busy = boxSecond.GetComponent<Grid>().Busy;
    ...
    {
        StartCoroutine("GameFinishedAction");
    }
