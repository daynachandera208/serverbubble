...
public class creatorBall : MonoBehaviour
{
    ...
    void ProcessGameDataFromString(string mapText)
    {
        ...
        foreach (string line in lines)
        {
            ...
            else if (line.StartsWith("STARS "))
            {
                ...
                Camera.main.GetComponent<mainscript>().moveCountForMultiColorBall = 0;
        ...
        for( int i = 0; i < rows; i++ )
        {
            for( int j = 0; j < columns; j++ )
            {
                ...
                if( mapValue > 0  )
                {
                    ...
                    createBall(GetSquare(roww, j).transform.position, (BallColor)mapValue, false, false,i, cMap[i * columns + j][0], cMap[i * columns + j][1], dCMap[i * columns + j],bGumMap[i * columns + j]);
                }
    ...
    public GameObject createBall( Vector3 vec, BallColor color = BallColor.random, bool newball = false, bool multiColor = false, int row = 1 ,int chainNo=0 ,int chainLevel=0 ,int doubleColor=0,int bGumNo=0)
    {
        GameObject b = null;
