...
public class LevelData
{
    ...
    public static int[][] chainMap = new int[11 * 70][];
    public static int[] bGumMap = new int[11 * 70];
    public static int[] doubleColorMap = new int[11 * 70];
    public static ArrayList allBGumNo = new ArrayList();
    ...
    static void ProcessGameDataFromXML( TextAsset xmlString )
    {
        ...
        for (int i = 0; i < creatorBall.rows; i++)
        {
            for (int j = 0; j < creatorBall.columns; j++)
            {
                ...
                if(value==-10) { // -10 for bubble eater
